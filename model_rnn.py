...
class Surge(nn.Module):
    def __init__(self,config):
        ...
        from layers import LSTMModel
        self.seq_model = LSTMModel(config.item_embedding_dim * 2, hidden_size=config.item_embedding_dim, num_layers=1, output_size=config.item_embedding_dim, device="cuda")
        self.task_fcn = Fcn_net(config.layer_sizes, mode='task', pre_size=config.item_embedding_dim * 2).cuda()  # task head MLP:
    ...
    def _build_embedding(self):
        ...
        self.item_embedding = nn.Embedding(config.item_num, config.item_embedding_dim, padding_idx=0)
    ...
    def forward(self, *data):
        ...
        # import ipdb; ipdb.set_trace()
        final_state = self.seq_model(X, self.mask.sum(-1).long())
        # import ipdb; ipdb.set_trace()
        model_output = torch.concat([final_state, self.target_pro_embedding], 1)
        # flatten sequence -> augru -> final state
        # alphas = torch.ones(X.shape[0], X.shape[1]).cuda()
        # # rnn_output = self.flatten(X, alphas)
        # rnn_output = self.augru(X, alphas)
        # final_state = rnn_output[range(
        #     rnn_output.shape[0]), self.mask.sum(-1).long()-1, :]
        # # self.augru(X, alphas)
